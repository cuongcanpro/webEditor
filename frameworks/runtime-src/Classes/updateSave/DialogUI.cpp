...
void DialogGUI::initUI()
{
	...
	bg = Sprite::create("res/Lobby/Dialog/table.png");
	addChild(bg);
	...
	auto title = Sprite::create("res/Lobby/Dialog/dialogTitle.png");
	bg->addChild(title);
	title->setPosition(bSize.width / 2, bSize.height - title->getContentSize().height/1.5f);

	content = Text::create("", "res/fonts/tahoma.ttf",17);
	...
	content->setTextColor(Color4B(132,140,220,255));
	content->ignoreContentAdaptWithSize(false);
	content->setContentSize(CCSize(bSize.width*0.9f, bSize.height / 2));
	bg->addChild(content);

	btnOK = Button::create("res/Lobby/Common/buttonOK.png", "res/Lobby/Common/buttonOK.png");
	btnCancel = Button::create("res/Lobby/Common/btnCancel.png", "res/Lobby/Common/btnCancel.png");
	btnOK->setTag(0);
	btnCancel->setTag(1);
