...
void LoadingGUI::initUI()
{
	...
	float _scale = size.width / ConfigReader::getInstance()->resolutionWidth;
	...
	bgAnim = Sprite::create(ConfigReader::getInstance()->bgAnim);
	bgAnim->setScaleX(size.width / bgAnim->getContentSize().width);
    bgAnim->setScaleY(size.height / bgAnim->getContentSize().height);
	bgAnim->setPosition(size.width / 2, size.height / 2);
	addChild(bgAnim, 99);
	
	// INTRO 
	intro = spine::SkeletonAnimation::createWithFile(ConfigReader::getInstance()->introJSON, ConfigReader::getInstance()->introAtlas, ConfigReader::getInstance()->introScale * _scale);
	intro->setAnimation(0, ConfigReader::getInstance()->introKey, false);
	intro->setPosition(size.width / 2, size.height / 2);
	intro->setEndListener(CC_CALLBACK_0(LoadingGUI::checkNoNetwork, this));
	addChild(intro, 100);
	
	borderAnim = Sprite::create(ConfigReader::getInstance()->borderAnim);
	borderAnim->setScaleX(size.width / borderAnim->getContentSize().width);
    borderAnim->setScaleY(size.height / borderAnim->getContentSize().height);
	borderAnim->setPosition(size.width / 2, size.height / 2);
	addChild(borderAnim, 101);
...
void LoadingGUI::introGame()
{
	...
	if (isIntro)
	{
		CocosDenshion::SimpleAudioEngine::getInstance()->playEffect(ConfigReader::getInstance()->introSound.c_str(), false);
		
		this->runAction(Sequence::create(DelayTime::create(ConfigReader::getInstance()->introTimeHide + ConfigReader::getInstance()->introTimePlay), CallFunc::create(CC_CALLBACK_0(LoadingGUI::checkNoNetwork, this)), NULL));
		intro->runAction(Sequence::create(DelayTime::create(ConfigReader::getInstance()->introTimePlay), Spawn::create(FadeOut::create(ConfigReader::getInstance()->introTimeHide), ScaleTo::create(ConfigReader::getInstance()->introTimeHide, 5), NULL), RemoveSelf::create(), NULL));
		bgAnim->runAction(Sequence::create(DelayTime::create(ConfigReader::getInstance()->introTimePlay), Spawn::create(FadeOut::create(ConfigReader::getInstance()->introTimeHide), ScaleTo::create(ConfigReader::getInstance()->introTimeHide, 5), NULL), RemoveSelf::create(), NULL));
		borderAnim->runAction(Sequence::create(DelayTime::create(ConfigReader::getInstance()->introTimePlay), Spawn::create(FadeOut::create(ConfigReader::getInstance()->introTimeHide), ScaleTo::create(ConfigReader::getInstance()->introTimeHide, 5), NULL), RemoveSelf::create(), NULL));
	}
	else
	{
		checkNoNetwork();
...
void LoadingGUI::checkAppversion() {
	...
	cRequest->setUrl(ConfigReader::getInstance()->urlService);
	...
	cocos2d::log("ServiceUpdate::%s?%s", ConfigReader::getInstance()->urlService.c_str(), data.c_str());
