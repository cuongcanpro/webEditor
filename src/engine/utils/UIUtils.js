var UIUtils = {
    ...
    doMappingChildren: function (node, targetAssign, exclude) {
        ...
        for (var i = 0; i < list.length; i++) {
            ...
            if (name && name != "") {
                ...
                targetAssign[name].rawPos = list[i].getPosition();
    ...
    initCCS: function (path, parent) {
        parent._rootNode = ccs.load(path, "").node;
    ...
    convertResTypeToBoosterType: function (resType) {
        if (resType == ResourceType.BOM_PHAO) return Constant.BoosterType.BOMB_ROCKET;
        else if (resType == ResourceType.HAT5) return Constant.BoosterType.RAINBOW;
        else if (resType == ResourceType.MAY_BAY) return Constant.BoosterType.PAPER_PLANE;
        else cc.warn("Invalid resType: " + resType);
    },

    convertBoosterTypeToResType: function (boosterType) {
        if (boosterType == Constant.BoosterType.BOMB_ROCKET) return ResourceType.BOM_PHAO;
        else if (boosterType == Constant.BoosterType.RAINBOW) return ResourceType.HAT5;
        else if (boosterType == Constant.BoosterType.PAPER_PLANE) return ResourceType.MAY_BAY;
