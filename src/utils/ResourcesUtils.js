...
ResourcesUtils.getSpritePathForResource = function (resourceType, num) {
    ...
    if (num != undefined){
        let resItem = new ResourceItem(resourceType, num);
        if (resItem.isCostumeResourceItem()){
            imgPath = imgPath.replace("@i@", resItem.getCostumeId());
        }
    }
...
ResourcesUtils.convertResTypeToBoosterType = function (resType) {
    if (resType == ResourceType.BOM_PHAO) return BoardConst.BoosterType.BOMB_ROCKET;
    else if (resType == ResourceType.HAT5) return BoardConst.BoosterType.RAINBOW;
    else if (resType == ResourceType.MAY_BAY) return BoardConst.BoosterType.PAPER_PLANE;
    else cc.warn("Invalid resType: " + resType);
};

ResourcesUtils.convertBoosterTypeToResType = function (boosterType) {
    if (boosterType == BoardConst.BoosterType.BOMB_ROCKET) return ResourceType.BOM_PHAO;
    else if (boosterType == BoardConst.BoosterType.RAINBOW) return ResourceType.HAT5;
    else if (boosterType == BoardConst.BoosterType.PAPER_PLANE) return ResourceType.MAY_BAY;
    else cc.warn("Invalid booster type: "+ boosterType);
};
