...
var BoardUtils = {
    getPriorityOfMatch: function (type) {
        ...
        var priority = BoardConst.MatchPriority[type];
    ...
    idxToGrid: function (index, gridWidth) {
        gridWidth = gridWidth || BoardConst.WIDTH;
    ...
    gridToIndex: function (row, col) {
        return row * BoardConst.WIDTH + col;
    },

    isCellOnBoard: function (row, col, width, height) {
        width = width || BoardConst.WIDTH;
        height = height || BoardConst.HEIGHT;
        if (row < 0 || row >= height || col < 0 || col >= width) return false;
        return true;
    },

    getGemTypeName: function (gemTypeId) {
        this.getKeyByValue(BoardConst.ElementType, gemTypeId);
    },
    ...
    getMovePriority: function (typeId) {
        ...
        let key = this.getKeyByValue(BoardConst.ElementType, typeId);
        if (key == null) return 0;
        return BoardConst.MovePriority[key] || 0;
    },

    getZOrderOfElement: function (typeId) {
        if (ElementUtils.isLayered(typeId)) typeId = BoardUtils.convertType(typeId).type;
        var key = this.getKeyByValue(BoardConst.ElementType, typeId);
        if (key == null) return 0;
        return BoardConst.zOrder[key] || 0;
    },
...
BoardUtils.convertType = function (type) {
    ...
    if (type < BoardConst.ElementType.GRASS) return {type: type};// min TypeId cua cac loai element co nhieu lop
    var num;
...
BoardUtils.initMatchPattern = function () {
    let listPattern = BoardConst.BasicMatchPattern;
    for (let type in listPattern) {
        let listRot = [1, 2, 3];
        if (type == BoardConst.ElementType.MATCH_4_V || type == BoardConst.ElementType.MATCH_4_H) listRot = [2];
    ...
    BoardConst.MatchPattern = BoardConst.BasicMatchPattern;
