...
### 5. MatchColorTakeDamageAction

**File:** `TakeDamageAction.js`  
**Extends:** `TakeDamageAction`

Element nhận damage nếu match có màu tương ứng.

#### Config Data:
```javascript
{
    _matchColor: 0 // Màu của match yêu cầu
}
```

#### Condition:
- `context.matchColor === _matchColor`
- `element.canTakeDamage(context.matchColor)` returns `true`
...
Thay thế toàn bộ strategy actions và UI của element hiện tại bằng strategy và UI của một element type khác (hoặc random từ list) mà không hủy bỏ instance của element hiện tại.

#### Config Data:
```javascript
{
    type: 1004 // Có thể là một Type ID (number) hoặc mảng các Type IDs [1004, 1005]
}
```
...
#### Execute Behavior:
1. Đợi animation hiện tại kết thúc (dựa vào `time` được truyền vào).
2. Random chọn 1 `targetType` mới từ mảng `configData.type` (nếu là mảng), hoặc sử dụng ID được set.
3. Tạo một template element tạm thời từ `targetType` để lấy cấu hình.
4. Xóa bỏ toàn bộ `actions` cũ của element.
5. Copy các thuộc tính cơ bản (`type`, `rawConfig`, `configData`, `size`, `layerBehavior`, `base actions`) từ template sang element hiện tại.
6. Khởi tạo lại toàn bộ `customAction` strategies mới dựa trên cấu hình của `targetType`.
7. Thay thế `ui` cũ bằng `ui` mới tương ứng với `targetType`.
8. Giữ nguyên các thuộc tính như `position`, `id`, `hitPoints` của element hiện tại.
